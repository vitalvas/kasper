...
// User represents a user account. The xml tags drive the documented schema
// for application/xml responses: a <user> root element with the id sent as an
// attribute. JSON responses keep using the json tags.
type User struct {
	XMLName   xml.Name  `json:"-" xml:"user"`
	ID        string    `json:"id" xml:"id,attr" openapi:"description=Unique user identifier,format=uuid,readOnly"`
	Name      string    `json:"name" xml:"name" openapi:"description=Full name,example=John Doe,minLength=1,maxLength=200"`
	Email     string    `json:"email" xml:"email" openapi:"description=Email address,format=email"`
	Role      string    `json:"role" xml:"role" openapi:"description=User role,enum=admin|editor|viewer"`
	CreatedAt time.Time `json:"created_at" xml:"created_at" openapi:"description=Account creation timestamp,readOnly"`
}
...
// ExportUsersQuery declares the query parameters for the export endpoint.
// It is registered with the explicit Parameters(QueryParamsFromStruct(...))
// form, which Query(...) wraps.
type ExportUsersQuery struct {
	Format string `query:"format,omitempty" openapi:"description=Override the Accept header,enum=json|xml|csv"`
	Since  string `query:"since,omitempty" openapi:"description=Only users created after this date,format=date"`
}

// AuditQuery is a shared filter applied at the group level via Query, so every
// operation in the group inherits the parameter.
type AuditQuery struct {
	Since string `query:"since,omitempty" openapi:"description=Filter entries after this date,format=date"`
}
...
// FileUploadForm describes a multipart file upload. The form tags drive the
// documented schema for the multipart/form-data content type, matching the
// field names mux.BindForm accepts.
type FileUploadForm struct {
	File        string `form:"file" openapi:"description=The file to upload"`
	Description string `form:"description,omitempty" openapi:"description=Optional file description,maxLength=500"`
}
...
func main() {
	...
	// Query parameters registered with the explicit Parameters + QueryParamsFromStruct
	// form (the Query shortcut used on listUsers wraps exactly this).
	users.Route(api.HandleFunc("/users/export", exportUsers).Methods(http.MethodGet)).
		...
		Parameters(openapi.QueryParamsFromStruct(ExportUsersQuery{})...).
	...
	// Audit log: deprecated route with default response link. The "since"
	// query parameter is inherited from the group's Query(AuditQuery{}).
