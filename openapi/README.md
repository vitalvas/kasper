...
Groups support `Tags`, `Security`, `Deprecated`, `Server`, `Parameter`, `Parameters`, `Query`, `ExternalDocs`, and the full response API: `Response`, `ResponseContent`, `ResponseDescription`, `ResponseHeader`, `ResponseLink`, `DefaultResponse`, `DefaultResponseDescription`, `DefaultResponseHeader`. Both `Route` and `Op` are available on a group.
...
When a Go type is generated by reflection, the property names are read from the struct tag that matches the content type, so the documented schema stays in sync with the matching `mux` binder:
...
For XML, the `XMLName` field sets the root element name and `,attr` fields are marked as XML attributes. Each tag falls back to the `json` tag and then the Go field name when its own tag is absent.
...
Query parameters are usually already declared on the struct passed to `mux.BindQuery`. `QueryParamsFromStruct` reads that struct and returns one `*Parameter` per field, so the documented parameters stay in sync with what the binder accepts. The `Query` method is a shortcut that registers them all in one call:
...
`Parameters(params ...*Parameter)` and `Query(v any)` are available on both operations and groups (group-level parameters are inherited by every operation in the group).
